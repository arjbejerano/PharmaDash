...
import { Activity, BarChart3, Boxes, LineChart, MapPinned, Menu, X } from 'lucide-react';
...
import { AnalyticsWorkspace, ForecastsWorkspace, LocationsWorkspace } from '@/components/OperationsWorkspace';

type Page = 'insights' | 'inventory' | 'forecasts' | 'locations' | 'analytics';
...
const Index = () => {
  ...
  const navigation = [
    ...
    { id: 'forecasts' as const, label: 'Forecasts', icon: LineChart },
    { id: 'locations' as const, label: 'Locations', icon: MapPinned },
  ...
  return <div className="pharma-theme min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">{page === 'insights' && <InsightsDashboard onOpenInventory={() => navigate('inventory')} />}{page === 'inventory' && <section><div className="mb-6"><p className="text-sm font-medium uppercase tracking-[0.2em] text-emerald-300">Operations</p><h2 className="mt-2 text-3xl font-semibold">Inventory management</h2><p className="mt-2 text-sm text-muted-foreground">Manage products, locations, purchase history, and product-level forecasts.</p></div><InventoryTable onSelectDrug={selectDrug} selectedDrugId={selectedDrugId} onStatsChange={setStats} /></section>}{page === 'forecasts' && <ForecastsWorkspace />}{page === 'locations' && <LocationsWorkspace />}{page === 'analytics' && <AnalyticsWorkspace />}</main>
